...
def process_video(video_path, label, detector, window=15):
    """Extract per-frame + temporal features from a video"""
    ...
    df = df.interpolate().bfill().ffill()  # fill any missing frames

    # Temporal features using rolling window
    ...
    # EAR velocity (how fast eyes are closing)
    ...
    # PERCLOS: percentage of eye closure in window
...
def main():
    ...
    for label, folder in [("awake", "data/sofia/awake"), ("sleepy", "data/sofia/sleepy")]:
        videos = sorted([f for f in os.listdir(folder) if f.endswith((".MOV", ".mp4", ".mov"))])
        print(f"Processing {len(videos)} {label.upper()} videos...")
        for i, vid in enumerate(videos):
            print(f"  [{i+1}/{len(videos)}] {vid}")
            df = process_video(os.path.join(folder, vid), label, detector)
            all_data.append(df)

    result = pd.concat(all_data, ignore_index=True)
    output = "fatigue_detection/classical/features_v2.csv"
    result.to_csv(output, index=False)
    print(f"\nDone! {len(result)} frames with {len(result.columns)} features")
